...
def export_all(outdir, rows, tempfits, figures):
    ...
    peak_rows=[]; metadata_rows=[]; curve_sheets={}; residual_sheets={}
    for label,spec,res in rows:
        for name,value in spec.metadata.items():
            metadata_rows.append({"Label":label,"Source":spec.source_name,"Key":name,"Value":value})
        for p in res.peaks:
    ...
    with pd.ExcelWriter(out/"PL_batch_results.xlsx",engine="openpyxl") as w:
        pd.DataFrame(peak_rows).to_excel(w,sheet_name="Peak_Results",index=False)
        pd.DataFrame(temp_rows).to_excel(w,sheet_name="Temperature_Fits",index=False)
        pd.DataFrame(metadata_rows,columns=["Label","Source","Key","Value"]).to_excel(w,sheet_name="Spectrum_Metadata",index=False)
        for k,v in curve_sheets.items(): v.to_excel(w,sheet_name=("Fit_"+k)[:31],index=False)
        for k,v in residual_sheets.items(): v.to_excel(w,sheet_name=("Res_"+k)[:31],index=False)
    ...
    payload={"spectra":[{"label":l,"spectrum":{"condition":s.condition,"sample_id":s.sample_id,"temperature_k":s.temperature_k,"source_name":s.source_name,"metadata":s.metadata},"fit":result_to_dict(r)} for l,s,r in rows],"temperature_fits":{l:{k:result_to_dict(v) for k,v in g.items()} for l,g in tempfits.items()}}
